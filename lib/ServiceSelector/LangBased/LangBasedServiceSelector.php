...
class LangBasedServiceSelector implements IServiceSelector, ICheck {
	...
	public function go() {
		...
		$classmap = $servicelocator->get('classmap');
		...
		$instance = $classmap->getInstanceByInterfaceName("Api\\IOutput", $name);
		if ($instance == null) {
	...
	public function checkDependencies() {
		$servicelocator = \Base3\ServiceLocator::getInstance();
		return array(
			"depending_services" => $servicelocator->get('classmap') == null || $servicelocator->get('language') == null ? "Fail" : "Ok"
		);
	}
}
